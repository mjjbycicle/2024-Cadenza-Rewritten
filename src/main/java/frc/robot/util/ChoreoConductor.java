...
public class ChoreoConductor {
  ...
  public Command scoreNotes(int... notes) {
    ...
    for (int i = 0; i < notes.length; i++) {
      ...
      Command afterDrive =
          afterDriveCommands.isEmpty() ? null : CommandsUtil.parallel(afterDriveCommands);
      ...
      if (shootOnTheFly) {
        noteCommands.add(
            intakeMoveShoot(
                pathName, false, aimSecondsBefore, afterDrive, 0, doBypass ? ampSide : null));
      } else {
        noteCommands.add(
            intakeMove(pathName, false, afterDrive, headingOverride, 0, doBypass ? ampSide : null));
      }
