...
public class Helpers {
  ...
  public static boolean withinTolerance(Translation2d translation, double maxErrorMeters) {
    return Math.abs(translation.getX()) <= maxErrorMeters
        && Math.abs(translation.getY()) <= maxErrorMeters;
  }
  ...
  /**
   * Custom version of that overwrites the given ChassisSpeeds instead of returning a new one.
   *
   * @param speeds
   * @param robotAngle
   */
  public static void fromRobotRelativeSpeedsOverwrite(ChassisSpeeds speeds, Rotation2d robotAngle) {
    ...
    var rotated =
        new Translation2d(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond).rotateBy(robotAngle);
    speeds.vxMetersPerSecond = rotated.getX();
    speeds.vyMetersPerSecond = rotated.getY();
  }

  static final Pose2d blankPose = new Pose2d();
  ...
  public static ChoreoTrajectory cropTrajectoryStart(
      ChoreoTrajectory trajectory, Double startTime) {
  ...
  public static ChoreoTrajectory cropTrajectory(
      ChoreoTrajectory trajectory, Double startTime, Double endTime) {
    ...
    for (ChoreoTrajectoryState state : trajectory.getSamples()) {
      ...
      if (check) {
        ...
        ChoreoTrajectoryState newState =
  ...
  /**
   * Sleeps for the specified number of milliseconds, without throwing an exception. Use this with
   * caution, as it can block the entire robot code.
   *
