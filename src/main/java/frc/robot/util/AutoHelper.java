...
import frc.robot.subsystems.drive.DrivetrainSubsystem.HeadingTarget;
...
public class AutoHelper {
  ...
  public static final double NOTE_X = 7.921848297119141 - 0.15;
  public static final double NOTE_Y_BASE = 7.472607612609863;
  public static final double NOTE_Y_DIFF = NOTE_Y_BASE - 5.788680553436279;
  ...
  public static Command shootIfNote() {
    // Disable skipping shoot command for now, in case note is almost fully loaded but not quite
    // hitting both sensors
  ...
  public static Command shootNoAim() {
    ...
    return Commands.waitUntil(mechanismsAligned).andThen(intake.feedShooterCommand());
  }
  ...
  public static Command shootWhenAligned(Double timeoutSeconds) {
    ...
    if (timeoutSeconds != null) {
      waitForAligned =
          Commands.race(Commands.waitUntil(allAligned()), Commands.waitSeconds(timeoutSeconds));
    } else {
      waitForAligned = Commands.waitUntil(allAligned());
    }

    return waitForAligned.andThen(intake.feedShooterCommand());
  }
  ...
  public static Command intakeSequence() {
    ...
    return Commands.sequence(
        intake.setExtended(RobotInfo.IntakeInfo.ExtenstionState.EXTENDED),
        new IntakeCommand(intake).withEndOnNoteDetected(true),
        intake.setExtended(RobotInfo.IntakeInfo.ExtenstionState.RETRACTED));
  }

  public static Command waitForIntake(double timeoutSeconds) {
    return Commands.race(
        Commands.waitUntil(intake.isAligned()), Commands.waitSeconds(timeoutSeconds));
  }
  ...
  public static Command intakeMove(
      String pathName,
      boolean resetPosition,
      Command afterDrive,
      Function<Double, Double> heading) {
  ...
  public static Command intakeMove(
    ...
    if (adjustableAmpSide == null) {
      ...
      driveSequence =
          buildAdjustableDriveSequence(
              pathName, resetPosition, afterDrive, heading, intakeWaitSeconds, adjustableAmpSide);
    }
    return Commands.deadline(driveSequence, intakeSequence());
  }

  public static Command intakeMoveAim(
      String pathName, boolean resetPosition, double aimSecondsBeforeEnd) {
    return intakeMoveAim(pathName, resetPosition, aimSecondsBeforeEnd, null);
  }

  public static Command intakeMoveAim(
      String pathName, boolean resetPosition, double aimSecondsBeforeEnd, Command afterDrive) {
  ...
  public static Command intakeMoveAim(
    ...
    if (adjustableAmpSide == null) {
      driveSequence =
          buildDriveSequence(
              path, resetPosition, afterDrive, aimHeading, Robot.isReal() ? 2.5 : 0.25);
    } else {
      driveSequence =
          buildAdjustableDriveSequence(
              pathName,
  ...
  public static Command intakeMoveAimBackIntake(
      String pathName,
      boolean resetPosition,
      double aimSecondsBeforeEnd,
      Command afterDrive,
  ...
  public static Command intakeMoveShoot(
      String pathName, boolean resetPosition, double aimSecondsBeforeEnd, Command afterDrive) {
    return intakeMoveShoot(
        pathName, resetPosition, aimSecondsBeforeEnd, afterDrive, Robot.isReal() ? 2.5 : 0.25);
  }
  ...
  public static Command intakeMoveShoot(
      ...
      double intakeWaitSeconds) {
    return intakeMoveShoot(
        pathName, resetPosition, aimSecondsBeforeEnd, afterDrive, intakeWaitSeconds, null);
  }
  ...
  public static Command intakeMoveShoot(
    ...
    if (adjustableAmpSide == null) {
      driveSequence =
          buildDriveSequence(path, resetPosition, afterDrive, aimHeading, intakeWaitSeconds);
    } else {
      driveSequence =
          buildAdjustableDriveSequence(
              pathName,
    ...
    return Commands.parallel(
        ...
        Commands.sequence(
            Commands.deadline(Commands.waitSeconds(aimStart), intakeSequence()),
  ...
  private static Command buildDriveSequence(
      ChoreoTrajectory path,
      boolean resetPosition,
      Command afterDrive,
      Function<Double, Double> heading,
      double intakeWaitSeconds) {
    return applyIntakeAndAfterDrive(
        drivetrain.followChoreoPath(path, resetPosition, heading), afterDrive, intakeWaitSeconds);
  }

  public static Command buildAdjustableDriveSequence(
      String pathName,
      boolean resetPosition,
      Command afterDrive,
      Function<Double, Double> heading,
  ...
  private static Command applyIntakeAndAfterDrive(
      Command driveSequence, Command afterDrive, double intakeWaitSeconds) {
        ...
        new DeferredCommand(
            () -> {
              ...
              try {
                ...
                startTime -= 0.2;
                return drivetrain.followChoreoPath(
                    Helpers.cropTrajectoryStart(path, startTime), false, null);
  ...
  public static Command driveDownCenterline(boolean ampSide) {
        ...
        drivetrain.run(
            () -> {
              ...
              double rotationSpeed =
                  drivetrain.calculateHeadingPID(pose.getRotation(), ampSide ? -90 : 90);
    ...
    drive =
        drive.beforeStarting(
            Commands.runOnce(() -> drivetrain.resetHeadingPID(HeadingTarget.POSE)));

    if (Robot.isSimulation()) return drive.withTimeout(1.5);
    return drive.until(intake::isNoteDetected);
  }
