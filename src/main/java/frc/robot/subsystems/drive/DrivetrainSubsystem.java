...
public class DrivetrainSubsystem extends SwerveDrivetrain implements Subsystem {
  ...
  private final SwerveRequest.ApplyChassisSpeeds autoClosedLoopRequest =
  ...
  public void initialize() {
    xPidController =
        new ProfiledPIDController(2.5, 0., 0, new TrapezoidProfile.Constraints(MAX_SPEED, 6));
    yPidController =
        new ProfiledPIDController(2.5, 0., 0, new TrapezoidProfile.Constraints(MAX_SPEED, 6));

    driverRotationPidController =
        new ProfiledPIDController(
            10, 0., 0, new TrapezoidProfile.Constraints(MAX_ANGULAR_RATE, MAX_ANGULAR_ACCEL));
    ...
    // rotationTuner = new PIDTuner(driverRotationPidController, ()-> m_targetHeading == null ? -1 :
    // m_targetHeading)
  ...
  public void periodic() {
    ...
    double headingDegrees =
        vision.getSpeakerHeadingType() == HeadingTarget.GYRO
            ? gyroDegrees
            : poseRotation.getDegrees();
    speakerError = vision.getSpeakerAngle() - MathUtil.inputModulus(headingDegrees, -180, 180);
    Logger.recordOutput("Speaker Error", speakerError);

    double acceptableSpeakerError =
        InterpolatorTables.getSpeakerTolerance(vision.getSpeakerDistance());
    Logger.recordOutput("Speaker Tolerance", acceptableSpeakerError);
    alignedToSpeaker =
        alignedToSpeakerDebouncer.calculate(Math.abs(speakerError) <= acceptableSpeakerError);
    alignedToSpeakerDebounced = alignedToSpeaker;

    Logger.recordOutput("Aligned to Speaker", alignedToSpeaker);
    Logger.recordOutput("Gyro Angle", gyroDegrees);
    Logger.recordOutput("Pose Angle", poseRotation.getDegrees());
    ...
    if (Robot.isAutonomousMode()) {
      Pose2d choreoTarget =
          new Pose2d(
              choreoX.getSetpoint(),
              choreoY.getSetpoint(),
              Rotation2d.fromRadians(choreoRotation.getSetpoint()));
      Logger.recordOutput("Choreo Pose Target", choreoTarget);
      Logger.recordOutput("Choreo X Error", choreoX.getPositionError());
      Logger.recordOutput("Choreo Y Error", choreoY.getPositionError());
      Logger.recordOutput(
          "Choreo Rotation Error", Units.radiansToDegrees(choreoRotation.getPositionError()));
    }
  ...
  public double calculateHeadingPID(Rotation2d heading, double targetDegrees) {
    ...
    return driverRotationPidController.calculate(
        heading.getRadians(), Math.toRadians(targetDegrees));
  }
  ...
  public double calculateHeadingPID(double headingDegrees, double targetDegrees) {
    ...
    return driverRotationPidController.calculate(
        Math.toRadians(headingDegrees), Math.toRadians(targetDegrees));
  }

  public double calculateXPID(double currentX, double targetX) {
    Logger.recordOutput("X Error", (targetX - currentX));
    double result = xPidController.calculate(currentX, targetX);
    Logger.recordOutput("X PID Result", result);
    return result;
  }
  ...
  public ChassisSpeeds calculateChassisSpeeds(Pose2d currentPose, Pose2d targetPose) {
    ...
    double thetaFeedback =
        calculateHeadingPID(currentPose.getRotation(), targetPose.getRotation().getDegrees());
  ...
  public void setTargetHeading(Double targetHeading, HeadingTarget type) {
    m_targetHeading =
        targetHeading == null ? null : MathUtil.inputModulus(targetHeading, -180, 180);
    m_targetHeadingType = type;
    if (targetHeading == null) {
      headingError = 0;
    } else {
      headingError = 999; // reset heading error to make sure we don't think we're at the new target
      // immediately
    }
  }
  ...
  public Command waitForTranslation(Translation2d translation, double distanceMeters) {
    final Translation2d flippedTranslation =
        new Translation2d(Constants.FIELD_WIDTH_METERS - translation.getX(), translation.getY());
    return Commands.waitUntil(
        () -> {
          Translation2d target = Robot.isRed() ? flippedTranslation : translation;

          Logger.recordOutput("waitForTranslation Target", target);
   ...
   * @param resetPosition If the robot's position should be reset to the starting position of the
   *     path
  ...
  public Command followChoreoPath(
      String pathName, boolean resetPosition, Function<Double, Double> rotationOverride) {
  ...
  public Command followChoreoPath(
    ...
    commands.add(
        rotationOverride != null
            ? choreoRotationCommand(trajectory, rotationOverride)
            : choreoSwerveCommand(trajectory));
  ...
  private Command choreoRotationCommand(
      ChoreoTrajectory trajectory, Function<Double, Double> rotationOverride) {
  ...
  /**
   * Returns a command that makes the robot follow a PathPlanner path using the PathPlannerLib
   * library.
   *
  ...
  /**
   * Creates a PathPlanner path that follows the given poses, and finishes with the given end
   * rotation.
   *
   * @param endRotation The rotation (i.e. heading) the robot should be facing at the end of the
   *     path
   * @param poses The poses that comprise the path.
   * @return A PathPlannerPath object that can be followed using {@link #followPath(PathPlannerPath,
   *     boolean) followPath}.
   */
  public PathPlannerPath createPath(Rotation2d endRotation, Pose2d... poses) {
    ...
    return new PathPlannerPath(
        ...
        new PathConstraints(
            ...
            MAX_SPEED_ACCEL,
            MAX_ANGULAR_RATE,
            MAX_ANGULAR_ACCEL), // The constraints for this path. If using a differential
        // drivetrain, the angular constraints have no effect.
        new GoalEndState(
            0.0, endRotation) // Goal end state. You can set a holonomic rotation here. If using a
  ...
  private void configurePathplanner() {
    AutoBuilder.configureHolonomic(
        ...
        new HolonomicPathFollowerConfig( // HolonomicPathFollowerConfig, this should likely live in
            ...
            Constants
                .DRIVETRAIN_TRACKWIDTH_METERS, // Drive base radius in meters. Distance from robot
            // center to the furthest module.
            new ReplanningConfig() // Default path replanning config. See the API for the options
            // here
            ),
        Robot::isRed, // Boolean supplier that controls when the path will be mirrored for the red
        // alliance
