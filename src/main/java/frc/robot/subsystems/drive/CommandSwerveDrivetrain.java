...
public class CommandSwerveDrivetrain extends SwerveDrivetrain implements Subsystem, Sendable {
  ...
  private final SwerveRequest.FieldCentricFacingAngle SOTFRequest =
  ...
  private SwerveRequest fieldCentricRequestSupplier() {
    double forwardsSpeed =
        Controls.DriverControls.SwerveForwardAxis.getAsDouble()
            * DriveConstants.CURRENT_MAX_ROBOT_MPS;
    double sidewaysSpeed =
        Controls.DriverControls.SwerveStrafeAxis.getAsDouble()
            * DriveConstants.CURRENT_MAX_ROBOT_MPS;
    double rotationSpeed =
        Controls.DriverControls.SwerveRotationAxis.getAsDouble()
            * DriveConstants.TELOP_ROTATION_SPEED;
  ...
  public Command pathfindCommand(Pose2d targetPose) {
    ...
    PathConstraints constraints = new PathConstraints(10, 5, 2 * Math.PI, 2 * Math.PI);
  ...
  public void periodic() {
    ...
    LimelightHelpers.PoseEstimate pose =
        validatePoseEstimate(
            LimelightHelpers.getBotPoseEstimate_wpiBlue("limelight"), Timer.getFPGATimestamp());
    if (pose != null) {
  ...
  private LimelightHelpers.PoseEstimate validatePoseEstimate(
      LimelightHelpers.PoseEstimate poseEstimate, double deltaSeconds) {
  ...
  private Command choreoSwerveCommand(ChoreoTrajectory trajectory) {
    return Choreo.choreoSwerveCommand(
        ...
        (ChassisSpeeds speeds) -> setControl(autoRequest.withSpeeds(speeds)),
        DriverStationUtil::isRed,
        this);
  }
        ...
        (poses) -> {
          ...
          if (poses.size() > 1) {
            ...
            for (var pose : poses.subList(1, poses.size())) {
              Pose2d delta =
                  new Pose2d(
                      pose.getTranslation().minus(lastPose.getTranslation()),
                      pose.getRotation().minus(lastPose.getRotation()));
              double curvature =
                  delta.getRotation().getRadians() / delta.getTranslation().getNorm();
  ...
  public void initSendable(SendableBuilder sendableBuilder) {
    ...
    sendableBuilder.addDoubleProperty("Heading", () -> getRotation2d().getDegrees(), null);
  }
