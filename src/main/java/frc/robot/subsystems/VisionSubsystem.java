...
public class VisionSubsystem extends SubsystemBase {
  ...
  public static final double SHOOTER_DEGREE_OFFSET =
      -6; // To account for shooter curving to the left
  ...
  private final Transform2d trapPoseTransform =
      new Transform2d(1.132, 0.32, Rotation2d.fromRadians(-0.005));
  ...
  public VisionSubsystem() {
    ...
    speakerPoseBlue = aprilTagFieldLayout.getTagPose(7).get().toPose2d().transformBy(speakerOffset);
    speakerTranslationBlue = speakerPoseBlue.getTranslation();

    speakerPoseRed = aprilTagFieldLayout.getTagPose(4).get().toPose2d().transformBy(speakerOffset);
  ...
  public void periodic() {
    if (firstPeriodic) {
      ...
      RobotContainer.instance.drivetrain.addVisionMeasurement(
          new Pose2d(), Timer.getFPGATimestamp());
      firstPeriodic = false;
    }
    ...
    PoseEstimate frontPose =
        validatePoseEstimate(null, 0); // Not using front limelight for odometry yet
    ...
    if (bestPose != null) {
      ...
      if (odometryEnabled) {
        RobotContainer.instance.drivetrain.addVisionMeasurement(
            bestPose.pose, bestPose.timestampSeconds);
      }
    }
    ...
    if (stageTarget != null) {
      ...
      Logger.recordOutput("Stage Tag ID", stageID);
    }
    ...
    if (shootOnTheFlyEnabled) {
      ...
      if (Math.abs(robotVel.vxMetersPerSecond) >= 0.03
          || Math.abs(robotVel.vyMetersPerSecond) >= 0.03) {
        ...
        isShootingOnTheFly =
            speakerTranslation.getX() != originalSpeakerTranslation.getX()
    ...
    Logger.recordOutput("Speaker Translation", speakerTranslation);
    Logger.recordOutput("SOTF Currently?", isShootingOnTheFly);
    ...
    if (speakerFiducial != null && directTagAiming && !isShootingOnTheFly) {
      speakerDegrees =
          RobotContainer.instance.drivetrain.getGyroscopeDegrees() - speakerFiducial.txnc;
    ...
    Logger.recordOutput("Speaker Distance", speakerDistance);

    // If we're shooting on the fly, we need to use the adjusted distance to the speaker for
    // shooting
    ...
    if (isShootingOnTheFly && Robot.TAG_HEIGHT_AIM) {
      // Shoot on the fly offsets the target position, so the tag height has to be estimated based
      // on the new position
    } else if (speakerFiducial != null) {
      speakerTagHeight = (speakerFiducial.tync + 24.85) * 19.3159;
    }
    Logger.recordOutput("Speaker Angle", speakerAngle);
    Logger.recordOutput("Speaker Angle Type", speakerAngleType.toString());
    Logger.recordOutput("Can See Speaker", canSeeSpeaker);
    Logger.recordOutput("Speaker Tag Height", speakerTagHeight != null ? speakerTagHeight : -1);
    Logger.recordOutput(
        "Speaker Tag Dist", speakerFiducial != null ? speakerFiducial.distToRobot : -1);
  }
  ...
  /**
   * Returns the appropriate value (tag height or distance) for the lookup tables based on
   * Robot.TAG_HEIGHT_AIM.
   *
   * @return Either the speaker tag height or the speaker distance, depending on
   *     Robot.TAG_HEIGHT_AIM.
   */
